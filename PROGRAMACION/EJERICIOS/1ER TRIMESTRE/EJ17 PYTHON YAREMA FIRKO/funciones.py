...
from categoria import insertar_peido_fechapedido, ver_pedido, update_pedido, eliminar_pedido, ver_detalle,update_pedido
def mostrar_tablas():
...
def ejecutar_opcion(tabla,opcion):
    ...
    if tabla == 4:
        if opcion == 1:
            ...
            print("\nListado de Categorías de pedido:")
            ...
            print("\nListado de Categorías de detalle:")
            for idpedido, idproducto, precio, unidades, descuento in categorias:
                print(f"idpedido: {idpedido}, idproducto: {idproducto}, precio:{precio}, unidades: {unidades}, descuento: {descuento}")
                 
        elif opcion == 3:
            idpedido = int(input("Ingrese idpedido a actualizar: "))
            fecha = input("Ingrese la nueva fecha del pedido: ")
            idproducto = int(input("Escribe el nuevo id del producto "))
            update_pedido(fecha, idpedido,idproducto)
            print("Categoría actualizada exitosamente.")
            
        elif opcion == 4:
            idpedido = int(input("Ingrese el idpedido a eliminar: "))
            eliminar_pedido(idpedido)
            print("Pedido eliminado exitosamente de la tabla producto y detalle.")
