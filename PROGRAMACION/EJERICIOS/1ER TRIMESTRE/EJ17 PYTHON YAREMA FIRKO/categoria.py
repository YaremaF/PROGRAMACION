...
def update_pedido(fecha_pedido,idpedido,idproducto):
    conexion = conectar()
    cursor = conexion.cursor()
    consulta = "UPDATE pedido SET fechapedido = %s WHERE idpedido = %s"
    cursor.execute(consulta, (fecha_pedido,idpedido,))
    print(idproducto,idpedido)

    consulta2 = "UPDATE detalle SET idproducto = %s WHERE idpedido = %s"
    cursor.execute(consulta2, (idproducto,idpedido,))
    print(idproducto,idpedido)
    conexion.commit()
    cursor.close()
    conexion.close()
...
def eliminar_pedido(idpedido):
    conexion = conectar()
    cursor = conexion.cursor()
    consulta2 = "DELETE FROM detalle WHERE idpedido = %s"
    cursor.execute (consulta2,(idpedido,))
